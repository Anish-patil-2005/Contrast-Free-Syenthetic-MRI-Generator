    ...
    if t1_file and flair_file and bravo_file:
        ...
        with st.spinner("Preparing data..."):
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                # Save uploaded files
                for key, uploaded in zip(
                    ["t1", "flair", "bravo"],
                    [t1_file, flair_file, bravo_file]
                ):
                ...
                # Modify transform to exclude target
        ...
        # ================= RUN MODEL =================
        fake, uncert, sid = run_synthesis_logic(
            inputs,
            n_samples=n_samples,
            prog_bar=prog_bar,
        ...
        st.divider()
        st.download_button(
            label="Download Generated NPY",
            data=fake.tobytes(),
            file_name=f"synth_{sid}.npy"
        )

    else:
        st.error("Please upload T1-Pre, FLAIR, and BRAVO sequences.")
